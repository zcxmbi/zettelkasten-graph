...
AI 帮你链接笔记，图论帮你审计 AI。把笔记变成社交网络——排名、导航、发现隐藏的知识聚类。14 种分析方法。
...
| 6 | **链接质量审查** | 检测不对称、传递缺失、权重偏差。包含图论自检和 LLM 蒸馏校准 |
...
```
脚本: python distill_attention.py "0000085-冲突层面"
  → LLM 读取源笔记 + 所有出链目标（纯内容，不展示权重）
  → 输出每个目标的注意力百分比
  → 与你归一化后的权重对比
  → 标记 >5% 的偏差
  → 人做最终修正
